...
void MemoryScheduler(virConnectPtr conn, int interval)
{
	...
	for (int i = 0; i < ndomains; i++)
	{
		...
		while (domainStats[i].unused < 100 && domainStats[i].actual < domainStats[i].maxLimit)
		{
			...
			for (int j = 0; j < ndomains; j++)
			{
				...
				if (j != i)
				{
					...
					} else if (domainStats[j].actual > 200)
					{
						...
						{
							fprintf(stderr, "Failed to set actual memory of %llu MB to sacrificed domain %d\n", domainStats[j].actual, j);
						}
						
						printf("Domain %d sacrificed actual memory of %llu MB\n", j, domainStats[j].actual);
						sacrificedVM = j;
						continue;
					}
				}
				...
				if (releasedMemory != 0)
				{
					...
					if(virDomainSetMemory(domains[i], domainStats[i].actual * 1024)){
						fprintf(stderr, "Failed to set actual memory of %llu MB to the starving domain %d\n", domainStats[i].actual, i);
					}
					printf("Starving domain %d now has memory of %llu MB from a sacrificed domain\n", j, domainStats[j].actual);
				}
			...
			if (sacrificedVM == -1)
			{
				if (hostFreeMemory > 200)
				{
					...
					printf("Starving domain %d now has memory of %llu MB from the host\n", i, domainStats[i].actual);
				}
		...
		while (domainStats[i].actual > (domainStats[i].maxLimit/4)){
			...
			domainStats[i].actual = domainStats[i].actual - releasedMemory;
			if(virDomainSetMemory(domains[i], domainStats[i].actual * 1024)){
				fprintf(stderr, "Failed to set actual memory of %llu MB to the bloated domain %d\n", domainStats[i].actual, i);
			}
			printf("Bloated domain %d now has memory of %llu MB after releasing memory.\n", i, domainStats[i].actual);
		}
	}
